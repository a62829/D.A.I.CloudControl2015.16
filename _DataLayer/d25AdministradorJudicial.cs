...
namespace _DataLayer
{
    public class d25AdministradorJudicial : DBcon
    {
        ...
        public DataTable getListaAdministradorJudicialForaDoProcesso(string id)
        {
            SqlDataReader reader;
            cmd.Parameters.AddWithValue("@id", id);
            cmd.CommandText = "SELECT * FROM administradorJudicial WHERE NOT EXISTS (SELECT * FROM administradorJudicialNoProcesso WHERE idAdministradorJudicial = administradorJudicial.id AND idProcesso = @id)";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = con;
            cmd.Connection.Open();
            reader = cmd.ExecuteReader();
        ...
        public void removerAdministradorJudicialDoProcesso(string idProcesso, string idAdministradorJudicial, string lastChangeBy)
        {
            ...
            cmd.Parameters.AddWithValue("@idAdministradorJudicial", idAdministradorJudicial);
            cmd.Parameters.AddWithValue("@lastChangeBy", lastChangeBy);
            cmd.CommandText = "DELETE FROM dbo.administradorJudicialNoProcesso WHERE idProcesso = @idProcesso AND idAdministradorJudicial = @idAdministradorJudicial";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = con;
            cmd.ExecuteNonQuery();
        }

        public DataTable getIdAdministradorJudicialNoProcesso(string idProcesso, string idAdministradorJudicial)
        {
            SqlDataReader reader;
            cmd.Parameters.AddWithValue("@idProcesso", idProcesso);
            cmd.Parameters.AddWithValue("@idAdministradorJudicial", idAdministradorJudicial);
            cmd.CommandText = "SELECT * FROM administradorJudicial WHERE NOT EXISTS (SELECT * FROM administradorJudicialNoProcesso WHERE idAdministradorJudicial = administradorJudicial.id AND idProcesso = @idProcesso)";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = con;
            cmd.Connection.Open();
            reader = cmd.ExecuteReader();
